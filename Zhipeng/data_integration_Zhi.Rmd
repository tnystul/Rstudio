...
```{r}
# install dataset
seur.data <- Read10X("/wynton/group/nystul/Rstudio/Zhipeng/WTnew2/filtered_feature_bc_matrix/")
...
# install dataset
seur.data <- Read10X("/wynton/group/nystul/J1910013/outs/filtered_feature_bc_matrix/")
...
combined <- merge(x = control, y = bam,  merge.data = T, project = "ov")
```

```{r init, results='hide', message=FALSE, fig.keep='none'}
# split the dataset into a list of two seurat objects (bam and WT)
...
```{r find.anchors}
ovary.anchors <- FindIntegrationAnchors(object.list = ifnb.list, anchor.features = features)
```

```{r integrate.data}
# this command creates an 'integrated' data assay
ovary.combined <- IntegrateData(anchorset = ovary.anchors)
```
...
DefaultAssay(ovary.combined) <- "integrated"
# Run the standard workflow for visualization and clustering
ovary.combined <- ScaleData(ovary.combined, verbose = FALSE)
ovary.combined <- RunPCA(ovary.combined, npcs = 30, verbose = FALSE)
ovary.combined <- RunUMAP(ovary.combined, reduction = "pca", dims = 1:30)
ovary.combined <- FindNeighbors(ovary.combined, reduction = "pca", dims = 1:30)
ovary.combined <- FindClusters(ovary.combined, resolution = 0.5)
saveRDS(ovary.combined, "ovary.combined.rds")
```

```{r viz, results='hide', message=FALSE}
# Visualization
p1 <- DimPlot(ovary.combined, reduction = "umap", group.by = "dataset")
p2 <- DimPlot(ovary.combined, reduction = "umap", label = TRUE, repel = TRUE)
p1 + p2
```
...
```{r split.dim}
DimPlot(ovary.combined, reduction = "umap", split.by = "dataset")
```
...
DefaultAssay(ovary.combined) <- "RNA"
ovary.markers <- FindConservedMarkers(ovary.combined, ident.1 = 6, grouping.var = "dataset", verbose = FALSE)
head(ovary.markers)
```
